...
// 모델 설정
const MODEL_CLASSIFY = 'mistralai/mistral-nemo'        // 분류용 (저렴)
...
- question: 질문 댓글 (궁금한 점, 도움 요청)
- suggestion: 제안 댓글 (개선점, 아이디어)
...
현재 응답해야 할 댓글의 유형: ${comment.type || 'other'}
적용할 애티튜드: ${attitude}
...
- "안녕하세요" 같은 형식적 인사 금지
- 자연스럽고 진정성 있는 응답
- 시청자 이름을 언급하지 마세요 (개인정보)
- 절대 방어적이거나 변명하는 태도 금지`
...
애티튜드별 응답 가이드:
${Object.entries(ATTITUDE_PROMPTS).map(([key, value]) => `- ${key}: ${value.split('\n')[0]}`).join('\n')}

중요:
- 각 댓글에 대해 200자 이내로 짧게 작성
- 이모지는 1-2개만 사용
- "안녕하세요" 같은 형식적 인사 금지
- 자연스럽고 진정성 있는 응답
- 시청자 이름을 언급하지 마세요 (개인정보)
...
[
  ...
  const userMessage = `다음 ${comments.length}개의 댓글에 대해 각각 응답을 작성해주세요:
